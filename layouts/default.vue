...
nav .nav-item { @apply flex items-center p-4 bg-teal-500 hover:bg-teal-400 cursor-pointer text-white }
h1 { @apply text-3xl font-semibold text-teal-500 }
h1.post-title { @apply text-6xl text-gray-700 }
h2 { @apply text-3xl font-semibold }
.section-title { @apply text-2xl font-bold text-teal-500 }

.btn { @apply rounded p-3 hover:shadow-lg hover:shadow-teal-400/40 cursor-pointer }
.btn-primary { @apply bg-teal-500 text-white }
.btn-secondary { @apply text-teal-500 border-2 font-bold }
input:not([type="checkbox"]) { @apply w-full border border-gray-400 rounded mt-1 p-3 focus:outline-3 focus:outline-teal-300 }
#loading { @apply size-16 my-4 mx-auto border-7 border-gray-200 rounded-full border-t-teal-500 animate-spin }
</style>